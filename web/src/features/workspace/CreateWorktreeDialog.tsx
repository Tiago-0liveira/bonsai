...
export function CreateWorktreeDialog() {
  ...
  const checkedOutBranches = useMemo(
    () => new Set(projectWorktrees.map((item) => item.branch)),
    [projectWorktrees],
  )
  ...
  const originBranches = useMemo(
    () =>
      [
        ...
        (branch, index, list) =>
          list.indexOf(branch) === index && !checkedOutBranches.has(branch.replace(/^origin\//, '')),
      ),
  ...
  const mergeTargets = [
    ...
    ...projectWorktrees
      .filter((item) => item.branch !== project?.defaultBranch)
  ...
  const submit = (event: FormEvent) => {
    ...
    createWorktree({
      sourceType,
      sourceRef,
      branchName: sourceType === 'new' ? branchName : undefined,
      tagId,
      mergeTargetBranch,
    })
  }
  ...
  return (
    <div className="absolute inset-0 z-[70] grid place-items-center bg-black/55 p-6 backdrop-blur-[2px]">
      <form
        onSubmit={submit}
        className="w-full max-w-[620px] overflow-hidden rounded-xl border border-[rgb(var(--border-strong))] bg-[rgb(var(--panel))] shadow-[0_28px_90px_rgb(0_0_0/.62)]"
      >
        ...
        <div className="space-y-5 p-4">
          <section>
            <div className="mb-2 text-[9px] font-semibold uppercase tracking-[.12em] text-[rgb(var(--muted-2))]">
              Branch source
            </div>
              ...
              {sourceOptions.map((option) => {
                ...
                return (
                  <button
                    ...
                    className={
                      'bonsai-focus rounded-lg border p-3 text-left transition-colors ' +
                      (active
                        ? 'border-[rgb(var(--purple)/.55)] bg-[rgb(var(--purple)/.10)]'
            ...
            {sourceType === 'new' ? (
              <>
                <label className="block">
                  <span className="mb-1.5 block text-[9px] font-semibold uppercase tracking-[.1em] text-[rgb(var(--muted-2))]">
                    New branch name
                  </span>
                  <input
                    autoFocus
                    ...
                    className="bonsai-focus h-9 w-full rounded-md border border-[rgb(var(--border))] bg-[rgb(var(--bg))] px-2.5 font-mono text-[11px] outline-none"
                  />
                </label>
                <label className="block">
                  <span className="mb-1.5 block text-[9px] font-semibold uppercase tracking-[.1em] text-[rgb(var(--muted-2))]">
                  ...
                  >
                    {mergeTargets.map((branch) => <option key={branch}>{branch}</option>)}
                  </select>
                </label>
              </>
            ) : (
              <label className="col-span-2 block">
                <span className="mb-1.5 block text-[9px] font-semibold uppercase tracking-[.1em] text-[rgb(var(--muted-2))]">
                  {sourceType === 'existing' ? 'Local branch' : 'Remote branch'}
                </span>
                <select
                  value={sourceRef}
                  onChange={(event) => setSourceRef(event.target.value)}
                  className="bonsai-focus h-9 w-full rounded-md border border-[rgb(var(--border))] bg-[rgb(var(--bg))] px-2.5 font-mono text-[11px] outline-none"
                >
                  {(sourceType === 'existing' ? localBranches : originBranches).map((branch) => (
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      borderColor: active ? presentation.foreground : presentation.border,
                      background: active ? presentation.background : 'rgb(var(--bg))',
                    }}
                  >
                    {tag.name}
                  </button>
          ...
          <label className="block">
            ...
            <span className="mt-1.5 block text-[9px] text-[rgb(var(--muted-2))]">
              Non-default targets are rendered underneath their parent branch on the canvas.
            </span>
          </label>
        </div>
        ...
        <div className="flex items-center justify-end gap-2 border-t border-[rgb(var(--border))] bg-[rgb(var(--bg)/.45)] px-4 py-3">
          <button
            type="button"
            onClick={() => setOpen(false)}
            className="bonsai-focus rounded-md px-3 py-2 text-[11px] text-[rgb(var(--muted))] hover:bg-[rgb(var(--panel-2))]"
          >
            Cancel
          </button>
          <button
            type="submit"
            disabled={!tagId || !sourceRef || (sourceType === 'new' && !branchName.trim())}
            className="bonsai-focus rounded-md border border-[rgb(var(--purple)/.45)] bg-[rgb(var(--purple)/.16)] px-3 py-2 text-[11px] font-medium text-[rgb(var(--text))] disabled:cursor-not-allowed disabled:opacity-40"
          >
            Create worktree
          </button>
        </div>
